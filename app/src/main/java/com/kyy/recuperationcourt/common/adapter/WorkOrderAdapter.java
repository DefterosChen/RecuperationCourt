package com.kyy.recuperationcourt.common.adapter;
...
public abstract class WorkOrderAdapter extends RecyclerView.Adapter<SmartViewHolder> implements ListAdapter {
    ...
    public WorkOrderAdapter(List<WorkOrder> list, @LayoutRes int layoutId) {
     ...
     * @param position
     */
    protected abstract void onBindViewHolder(SmartViewHolder holder, WorkOrder model, int position);
    ...
    public WorkOrderAdapter setOnItemClickListener(SmartViewHolder.OnItemClickListener listener) {
        mOnItemClickListener = listener;
        return this;
    }

    public WorkOrderAdapter setOnItemLongClickListener(SmartViewHolder.OnItemLongClickListener listener) {
        mOnItemLongClickListener = listener;
        return this;
    }

    public WorkOrderAdapter refresh(Collection<WorkOrder> collection) {
        if (collection != null) {
            mList.clear();
    ...
    public WorkOrderAdapter loadMore(Collection<WorkOrder> collection) {
        if (collection != null) {
            mList.addAll(collection);
            notifyChanged();
        }
    ...
    public WorkOrderAdapter load(WorkOrder item) {
        if (item != null) {
            mList.add(item);
            notifyChanged();
        }
    ...
    public void replace(int position, WorkOrder item) {
    ...
    public void replaceNotNotify(int position, WorkOrder item) {
        if (position > -1 && position < mList.size() && item != null) {
            mList.set(position, item);
        }
    }
     ...
     * @return
     */
    public List<WorkOrder> getListData() {
        return mList;
    }
